...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="bg-white p-7 rounded-xl border border-gray-200 relative group transition-all duration-200 ease-in-out hover:border-gray-300 hover:-translate-y-1 shadow-sm"
    >
      <h3 className="text-2xl font-semibold text-gray-900 mb-2">{title}</h3>
      <p className="text-gray-600 mb-4">{oneLiner}</p>
      <div className="mb-4">
      ...
      {techChips && techChips.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-4 mb-4">
          {techChips.map((chip, idx) => (
            <span key={idx} className="inline-block px-3 py-1 rounded-full bg-gray-200 text-gray-900 text-xs">
              {chip}
            </span>
          ))}
        </div>
      ...
      <div className="flex gap-4 mt-6">
        {demoLink && (
          <Link href={demoLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-primary hover:text-gray-900 transition-colors duration-300">
            Demo
        ...
        {githubLink && (
          <Link href={githubLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-primary hover:text-gray-900 transition-colors duration-300">
            GitHub
